...
//                              BuildDetector()
//------++++++------++++++------++++++------++++++------++++++------++++++------
TMS_AmBeBoratedPEShieldGamma::TMS_AmBeBoratedPEShieldGamma(){
  ...
  G4Box * outerVolume_box = new G4Box("outer_volume_box", 15.5*2.54*cm, 15.5*2.54*cm, total_volume_half_height);
...
//                                              0,0,true);
  ...
  BaccDetectorComponent * single_lead_brick_1 = new BaccDetectorComponent(0,
                                             G4ThreeVector(5.*2.54*cm,2.*2.54*cm,2.5*2.54*cm),
 ...
 half_lead_brick_log->SetVisAttributes( BACCmaterials->EJ200Vis() );
  ...
  BaccDetectorComponent * half_lead_brick_1 = new BaccDetectorComponent(0,
                                             G4ThreeVector(4.*2.54*cm,0.,2.5*2.54*cm),
                                             half_lead_brick_log,
                                             "half_lead_brick_1",
                                             outerVolume_log,
                                             0,0,true);
...
//                                             0,0,true);
 ...
 G4SubtractionSolid * bpe_shield_sub = new G4SubtractionSolid("bpe_shield_sub",
                                                              ...
                                                              0,
                                                              G4ThreeVector(3.*2.54*cm,0.,2.*2.54*cm) );
...
//                                             0,0,true);
 ...
 G4Box * lead_wall_box = new G4Box("lead_wall_box",
				     ...
				     4.*2.54*cm,
				     10.*2.54*cm);
 G4Box * lead_wall_hole = new G4Box("lead_wall_hole_box",
				     3.*2.54*cm,
				     0.5*2.54*cm,
				     0.5*2.54*cm);
 G4SubtractionSolid * lead_wall_sub = new G4SubtractionSolid("lead_wall_sub",
			  	     lead_wall_box,
				     lead_wall_hole,
				     0,
				     G4ThreeVector(0.,0.,0.5*2.54*cm));
 G4LogicalVolume * lead_wall_log = new G4LogicalVolume( lead_wall_sub, BACCmaterials->Lead(), "lead_wall_log");
 lead_wall_log->SetVisAttributes( BACCmaterials->TestPurpleVis() );
 BaccDetectorComponent * lead_wall = new BaccDetectorComponent(0,
				     G4ThreeVector((13.+0.2+1.)*2.54*cm,0.,-0.5*2.54*cm),
				     lead_wall_log,
				     "lead_wall",
				     outerVolume_log,
				     0,0,true);
