...
//
// Change log:
// Jim Dobson, 6th Dec 16: removed hardcoded E_PURITY 2*m definition
//   and instead set dynamically based on BaccManager::GetDriftElecAttenuation() 
//
////////////////////////////////////////////////////////////////////////
...
// is but an alpha version of the electroluminescence code
	...
	//if(baccManager->GetGasRun()) z_start=theScintProcess->GAT;
	...
	if ( Phase == kStateLiquid && x0[2] > (theScintProcess->GAT-5*mm) && theScintProcess->GAT != 0 ) {
 ...
 GetConstProperty("ELECTRICFIELDSURFACE")/(volt/cm)),MillerDriftSpeed,z1));
	...
	if ( theScintProcess->WIN>0 && theScintProcess->TOP>0 && theScintProcess->ANE>0 && theScintProcess->SRF>0 && theScintProcess->GAT>0 && theScintProcess->CTH>0 && theScintProcess->BOT>0 && theScintProcess->PMT>0 ) {
	...
	if ( Density > 0. && Phase == kStateGas && (x0[2] > z_start ||
	  ...
	  //if ( baccManager->GetGasRun() ) ExtractEff = 1;
                                          ...
                                          G4ForceCondition* condition)
{
  ...
  if ( (z1==2 || z1==10 || z1==18 || z1==36 || z1==54) &&
    ...
    if ( aTrack.GetPosition()[2] < (theScintProcess->GAT-5*mm) || aTrack.GetWeight() == 2 )
      return baccManager->GetDriftElecAttenuation();
   ...
   aMaterial->GetState()== kStateGas && aMaterial->GetDensity()>0.0*(g/cm3)) {
    ...
    mfp*=cm/ConvertEff[z1];if(mfp<0)mfp=0;if(mfp>theScintProcess->GASGAP/2.7)mfp=theScintProcess->GASGAP/exp(1);
