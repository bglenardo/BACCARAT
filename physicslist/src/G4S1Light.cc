...
// Version:     1.00 alpha 1d LUX edition
// Created:     Wednesday, October 15, 2014 11:33 P.M.
...
//
// Change log:
// 2017/02/22 - saves creation position of thermal electron into storage in BaccManager (TD & AC)
//
////////////////////////////////////////////////////////////////////////
#include <math.h>
...
// this is the most important function, where all light & charge yields happen!
{
	...
	if ( WIN>0 && TOP>0 && ANE>0 && SRF>0 && GAT>0 && CTH>0 && BOT>0 && PMT>0 ) {  //Note that this could be a problem for the System Test, where the zero point in Z is defined somewhere in the liquid region
...
//          ScintillationYield = 1 / ((21.717-2.7759*Density)*eV);
          ScintillationYield = 1 / ( 13.7*eV );
	  ExcitationRatio = (0.059813+0.031228*Density) *(1.-exp(-aStep.GetTotalEnergyDeposit()/keV));
	...
	if ( (z1 == z2 && pDef->GetParticleType() == "nucleus") || 
	  ...
	  if ( ElectricField == 0 && Phase == kStateLiquid ) {
 	  ...
 	  ExcitationRatio = 1.240 *
 	    pow ( ElectricField, -0.0472 ) * (1.-exp(-239*epsilon));
	...
	if( fabs(TotalEnergyDeposit-InitialKinetEnergy)<safety || 
	  ...
	  for(i=0;i<j;i++) {
	    ...
	    if ( (delta < R0 && !fVeryHighEnergy) || z2==z1 ) {
		...
		if ( abs ( z1 - z2 ) && //electron recoil
		  ...
		  G4double tibMain = 0.6347*exp(-0.00014*ElectricField);
		  G4double tibEdep = -0.373*exp(-ElectricField*0.001/tibMain)+1.5;
		  G4double tibCurlA = 10.*pow(ElectricField,-0.04)*exp(18./ElectricField);
		  G4double tibCurlB = 0.188*pow(ElectricField,(1./3.));
		  G4double tibCurlZ = 1.-pow(ElectricField,0.2147)+3.;
		  ...
		  ThomasImel =
                     0.0554 * pow( ElectricField, -0.0620 );		  
		} //end NR information
		...
		if(recombProb>1) recombProb=1;
	      ...
	      //just like Doke: simple binomial distribution
 	      G4double FanoFactor;
              if( YieldFactor < 1) { // NR
                FanoFactor = 0.0068 * NumIons;
              }
              else { // ER
                FanoFactor = 0.0070 * NumIons;
              }

 	      if ( FanoFactor < 0. ) FanoFactor = 0.;
              if (recombProb != recombProb) {
                  recombProb = 0.;
              }
 	      NumElectrons = modPoisRnd(NumIons*(1.-recombProb),FanoFactor);
                 
// 	      if ( NumIons <= 50 )
// 		NumElectrons = BinomFluct(NumIons,1.-recombProb);
 	      if(NumElectrons>(NumExcitons+NumIons))NumElectrons=(NumExcitons+NumIons);
	    ...
	    // the Thomas-Imel block of code above was not executed
	      ...
	      if ( fAlpha || z1 == z2 ) { //bi-excitonic quenching due to high dE/dx
                if (z2 == z1) biExc = 1./(1 + 3.32*pow(epsilon,1.141));
		...
		if ( Phase == kStateLiquid )
	      ...
	      //Code segment removed to aid S2-vertex association, the need for it is likely out-dated.
		...
		if(aQuantum->GetDefinition()->GetParticleName()=="thermalelectron") 
		  baccManager->AddS2Position(aSecondaryTime,x0);
	      }
	    }
