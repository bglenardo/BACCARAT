...
int main(int argc, char *argv[]){
  ...
  if (argc != 3) {
    ...
    cout << "./LSDetectorHitSelection <path/to/rootfile> <outfileName>" << endl;
    return 0;
  }
     ...
     for(int tr=0; tr<evt->tracks.size(); tr++) { 
         ...
         for(int st=0; st<evt->tracks[tr].steps.size(); st++) { 
               ...
               if( evt->tracks[tr].steps[st].sVolumeName.find("tof_target")!=std::string::npos ||
                   evt->tracks[tr].steps[st].sVolumeName.find("ls_target")!=std::string::npos ||
                   evt->tracks[tr].steps[st].sVolumeName.find("psd_target")!=std::string::npos ){  
