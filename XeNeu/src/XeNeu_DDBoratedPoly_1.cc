...
//------++++++------++++++------++++++------++++++------++++++------++++++------
XeNeu_DDBoratedPoly_1::XeNeu_DDBoratedPoly_1()
{


  double volume_half_width = (0.5 * cm + 4 * 1. * 2.54 * cm)/2.;
  double volume_half_height = 7. * 2.54 * cm;
  double volume_half_length = 27. * cm / 2.;

  double b_poly_sheet_half_width = 1. * 2.54 * cm / 2.;
  double b_poly_sheet_half_length = volume_half_length - 0.75 * 2.54 * cm;
  double b_poly_sheet_half_height = volume_half_height;

  double lead_sheet_half_width = 0.5/2. * cm;
  double lead_sheet_half_length = 27./2. * cm;
  ...
  //example box -- make it into a minimal working example
   G4Box * boratedPoly1Volume_box = new G4Box("shieldingVolume", 
                                             ...
                                             volume_half_length, 
                                             volume_half_height);
  boratedPoly1Volume_log = new G4LogicalVolume( boratedPoly1Volume_box, BACCmaterials->Vacuum(), "boratedPoly1Volume");
  boratedPoly1Volume_log->SetVisAttributes( BACCmaterials->VacuumVis() );
  ...
  double sheet_positionY = volume_half_length - b_poly_sheet_half_length;
  for(int i=0; i<4; i++) {
